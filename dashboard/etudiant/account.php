...
$stmt = $pdo->prepare("SELECT nom, username, prenom, role, photo, fond_ecran FROM users WHERE id_users = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    if (!empty($_POST['new_password'])) {
        ...
        $pdo->prepare("UPDATE users SET password_hash = ? WHERE id_users = ?")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_photo']) && isset($_FILES['photo'])) {
    ...
    if (in_array($file_type, $allowed_types) && $file_size <= $max_size) {
        if (move_uploaded_file($_FILES['photo']['tmp_name'], $target_file)) {
            $pdo->prepare("UPDATE users SET photo = ? WHERE id_users = ?")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_background'])) {
    ...
    $pdo->prepare("UPDATE users SET fond_ecran = ? WHERE id_users = ?")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_info'])) {
    ...
    $pdo->prepare("UPDATE users SET nom = ?, prenom = ? WHERE id_users = ?")
...
<?php include $_SERVER['DOCUMENT_ROOT'] . '/ECE-Cine/includes/footer.php'; ?>
